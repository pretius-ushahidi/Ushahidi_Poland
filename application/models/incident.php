...
class Incident_Model extends ORM
{
	...
	*/
	public static function get_total_reports($approved=false)
	{
		if($approved)
		{
			$count = ORM::factory('incident')->where('incident_active', '1')->where('incident_archived', '0')->count_all();
		}else{
			$count = ORM::factory('incident')->where('incident_archived', '0')->count_all();
		}
	...
	*/
	public static function get_oldest_report_timestamp($approved=true)
	{
		if($approved)
		{
			$result = ORM::factory('incident')->where('incident_active', '1')->where('incident_archived', '0')->orderby(array('incident_date'=>'ASC'))->find_all(1,0);
		}else{
			$result = ORM::factory('incident')->where('incident_active', '0')->where('incident_archived', '0')->orderby(array('incident_date'=>'ASC'))->find_all(1,0);
		}
	...
	static function get_incidents_by_interval($interval='month',$start_date=NULL,$end_date=NULL,$active='true',$media_type=NULL)
	{
		...
		$query_text = 'SELECT UNIX_TIMESTAMP(' . $select_date_text . ') AS time,
					   ...
					   WHERE incident_archived = 0 AND incident_active IN (' . $active_filter .')' .
		$general_filter .'
