    ...
    function index()
    {
        ...
        if (is_numeric($category_id) && $category_id != '0')
        {
			...
			$markers = ORM::factory('incident')
				...
				->where('incident.incident_active = 1 AND incident_archived = 0 AND ('.$this->table_prefix.'incident_category.category_id = ' . $category_id . ' ' . $where_child . ')' . $where_text)
				->find_all();
        ...
        else
        {
			...
			$markers = ORM::factory('incident')
				...
				->with('location')
				->join('media', 'incident.id', 'media.incident_id','LEFT')
				->where('incident.incident_active = 1 AND incident_archived = 0 '.$where_text)
				->find_all();
        }
	...
	public function single($incident_id = 0)
	{
		...
		if ($marker->loaded)
		{
			...
			((ACOS(SIN($latitude * PI() / 180) * SIN(l.`latitude` * PI() / 180) + COS($latitude * PI() / 180) * COS(l.`latitude` * PI() / 180) * COS(($longitude - l.`longitude`) * PI() / 180)) * 180 / PI()) * 60 * 1.1515) AS distance 
			 FROM `".$this->table_prefix."incident` AS i INNER JOIN `".$this->table_prefix."location` AS l ON (l.`id` = i.`location_id`) INNER JOIN `".$this->table_prefix."incident_category` AS ic ON (i.`id` = ic.`incident_id`) INNER JOIN `".$this->table_prefix."category` AS c ON (ic.`category_id` = c.`id`) WHERE i.incident_active=1 AND incident_archived = 0 $filter 
			HAVING distance<='62'
			 ORDER BY i.`id` ASC ");
