...
class Api_Controller extends Controller {
	...
	*/
	...
	function _getIncidents($where = '', $limit = ''){
		...
		$query = "SELECT i.id AS incidentid,i.incident_title AS incidenttitle,"
				...
				."i.incident_archived AS incidentarchived, "
	...
	*/
	function _3dkml(){
		...
		$incidents = json_decode($this->_getIncidents('WHERE incident_active=1 AND incident_archived=0'));
	...
	*/
	function _getGeographicMidpoint(){
		...
		$json_latlon = array();
		$retJsonOrXml = '';
		$query = 'SELECT AVG( latitude ) AS avglat, AVG( longitude ) AS avglon FROM '.$this->table_prefix.'location WHERE id IN (SELECT location_id FROM '.$this->table_prefix.'incident WHERE incident_active = 1 AND incident_archived = 0 )';
		$items = $this->db->query($query);
	...
	*/
	function _getIncidentCount(){
		...
		$json_count = array();
		$retJsonOrXml = '';
		$query = 'SELECT COUNT(*) as count FROM '.$this->table_prefix.'incident WHERE incident_active = 1 AND incident_archived = 0';
		$items = $this->db->query($query);
	...
	function _incidentsByAll($orderfield,$sort,$limit) {
		$where = "\nWHERE i.incident_active = 1 AND incident_archived = 0 ";
	...
	function _incidentById($id){
		$where = "\nWHERE i.id = $id AND i.incident_active = 1 AND incident_archived = 0 ";
	...
	function _incidentsByLatLon($lat, $orderfield,$long,$sort){
		$where = "\nWHERE l.latitude = $lat AND l.longitude = $long AND i.incident_active = 1 AND incident_archived = 0 ";
		$sortby = "\nORDER BY $orderfield $sort ";
		$limit = "\n LIMIT 0, $this->list_limit";
		return $this->_getIncidents($where,$sortby,$limit);
	}
	...
	function _incidentsByLocationId($locid,$orderfield,$sort){
		$where = "\nWHERE i.location_id = $locid AND i.incident_active = 1 AND incident_archived = 0 ";
		$sortby = "\nORDER BY $orderfield $sort";
		$limit = "\nLIMIT 0, $this->list_limit";
		return $this->_getIncidents($where.$sortby, $limit);
	}
	...
	function _incidentsByLocationName($locname,$orderfield,$sort){
		$where = "\nWHERE l.location_name = '$locname' AND
				i.incident_active = 1 AND incident_archived = 0 ";
		$sortby = "\nORDER BY $orderfield $sort";
		$limit = "\nLIMIT 0, $this->list_limit";
		return $this->_getIncidents($where.$sortby, $limit);
	}
	...
	function _incidentsByCategoryId($catid,$orderfield,$sort){
		...
		$where = $join."\nWHERE c.id = $catid AND i.incident_active = 1 AND incident_archived = 0 ";
		$sortby = "\nORDER BY $orderfield $sort";
		$limit = "\nLIMIT 0, $this->list_limit";
		return $this->_getIncidents($where.$sortby, $limit);
	}
	...
	function _statistics(){
		...
		$incidents_unapproved = ORM::factory('incident')->where('incident_active', '0')->where('incident_archived', '0')->count_all();
