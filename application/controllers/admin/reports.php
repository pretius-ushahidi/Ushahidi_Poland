...
class Reports_Controller extends Admin_Controller
{
    ...
    */
	function index($page = 1)
	{
		...
		if (!empty($_GET['status']))
		{
			...
			{
				$filter = 'incident_active = 0 AND incident_archived = 0';
			}
			elseif (strtolower($status) == 'v')
			{
				$filter = 'incident_verified = 0 AND incident_archived = 0';
			}
			else
			{
				$status = "0";
				//$filter = '1=1';
                // incidents are now archived
				$filter = 'incident_archived = 0';
			}
		}
		else
		{
	    ...
	    if ($_POST)
	    {
			...
			if ($post->validate())
	        {
                ...
                } elseif ($post->action == 'r')	//Archive action
				{
					foreach($post->incident_id as $item)
					{
						$update = new Incident_Model($item);
						if ($update->loaded == true) {
							$incident_id = $update->id;
                            Kohana::log('info', 'Archiving incident (instead of deleting it), id: ' . $incident_id);
                            $update->incident_archived = 1;
                            $update->save();
                        }
                    }
                    $form_action = strtoupper(Kohana::lang('ui_admin.archived'));
                }
				$form_saved = TRUE;
			}
			else
    ...
    */
    ...
    function notify() {
        ...
        $notifies = ORM::factory('alert')->where('alert_type', '2')->where('alert_confirmed', '1')->orderby('alert_recipient', 'asc')->find_all();
        $this->template->content = new View('admin/reports_notify');
		$this->template->content->title = 'Powiadomienia';
		$this->template->content->notifies = $notifies;
