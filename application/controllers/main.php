...
class Main_Controller extends Template_Controller {
    ...
    public function index()
    {
		...
		$query = $db->query('SELECT incident_date, count(*) AS incident_count FROM '.$this->table_prefix.'incident WHERE incident_active = 1 AND incident_archived = 0 GROUP BY DATE_FORMAT(incident_date, \'%Y-%m\') ORDER BY incident_count DESC LIMIT 1');
        ...
        $query = $db->query('SELECT DATE_FORMAT(incident_date, \'%Y\') AS incident_date FROM '.$this->table_prefix.'incident WHERE incident_active = 1 AND incident_archived = 0 GROUP BY DATE_FORMAT(incident_date, \'%Y\') ORDER BY incident_date');
